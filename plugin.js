...
【重要：单章节输出要求】\n你每次只输出一个章节的内容。不要将故事拆分成多个章节。将所有内容写在一个章节中，通过场景转换、时间跳跃等叙事手法自然过渡。如果故事较长，用空行或「※」分隔不同场景，而不是拆成多个chapter。\n\n【输出格式】\n请返回纯JSON格式：\n{ \"chapter\": { \"title\": \"\", \"content\": [{ \"type\": \"p|dialogue|narrator\", \"text\": \"\" }] }, \"comments\": [{ \"name\": \"\", \"text\": \"\", \"time\": \"\", \"likes\": 0, \"replyTo\": \"\" }], \"annotations\": [{ \"paragraphIndex\": 0, \"quotes\": \"\", \"notes\": [{ \"name\": \"\", \"text\": \"\" }] }], \"continuation_summary\": \"\" }\n\n其中 continuation_summary 是150字左右的续写摘要，概括本篇内容与情感走向，供后续连载追更时作为上下文传入。\n字数要求：[WORD_COUNT_PLACEHOLDER]",
  ...
  function toggleDebugPanel() {
    ...
    toolbar.innerHTML = '<span>hofter debug (v1.5.0)</span><div><span style="cursor:pointer;margin-left:10px;color:#ff0;" onclick="window.__hofter.clearDebug()">CLEAR</span><span style="cursor:pointer;margin-left:10px;color:#0ff;" onclick="window.__hofter.copyDebug()">COPY</span><span style="cursor:pointer;margin-left:10px;color:#f66;" onclick="window.__hofter.toggleDebug()">X</span></div>';
